...
Crawler.prototype._buildHttpRequest = function _buildHTTPRequest(options) {
	...
	var doRequest = function (err) {
      ...
      let requestTimeoutTimer = null
			const req = request(_.pick.apply(self, [ropts].concat(requestArgs)), function (error, response) {
      ...
      if (options && options.timeout) {
        requestTimeoutTimer = setTimeout(function() {
          log('debug', 'request timeout, abort it' + options.uri)
          req.abort();
          self._onContent(new Error("TIMEOUT"), options);
        }, options.timeout)
      }
		}
	};
